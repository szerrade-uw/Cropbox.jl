...
```@example Cropbox
dirname(dirname(pathof(Cropbox))
```
\
...
```@example Cropbox
first(CSV.read(download("https://raw.githubusercontent.com/junhyukjeon/Cropbox.jl/documentation/docs/src/tutorials/weather.csv"), DataFrame), 3)
```
\
